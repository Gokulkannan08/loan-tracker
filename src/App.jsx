...
function DoughnutChart({ principal, interest }) {
  const data = [
    ...
    <div className="flex items-center gap-5">
      <ResponsiveContainer width={160} height={160}>
        <PieChart>
          ...
          <Tooltip formatter={(v) => fmtFull(v)} contentStyle={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 8, fontSize: 12 }} />
        </PieChart>
      </ResponsiveContainer>
  ...
  return (
    <ResponsiveContainer width="100%" height={180}>
      <BarChart data={data} margin={{ top: 5, right: 5, left: -15, bottom: 0 }}>
        ...
        <XAxis dataKey="fy" tick={{ fontSize: 10, fill: "#64748b" }} tickFormatter={(v) => `FY${String(v).slice(-2)}`} />
        <YAxis tick={{ fontSize: 10, fill: "#64748b" }} tickFormatter={(v) => v >= 1e5 ? `${(v / 1e5).toFixed(0)}L` : v} />
        <Tooltip formatter={(v) => fmtFull(v)} contentStyle={{ background: "#1e293b", border: "1px solid #334155", borderRadius: 8, fontSize: 12 }} labelFormatter={(v) => `FY ${v}-${(v + 1).toString().slice(-2)}`} />
...
export default function App() {
  ...
  const [loanType, setLoanType] = useState("floating");
              ...
              <Card className="border-t-[3px] border-t-blue-500">
                ...
                <CardContent className="flex flex-col gap-3">
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <ToggleGroup type="single" value={loanType} onValueChange={(v) => v && setLoanType(v)}>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <ToggleGroup type="single" value={coBorrowers} onValueChange={(v) => v && setCoBorrowers(v)}>
                  ...
                  <Field label="Tax Bracket" suffix="%">
                    <Input type="number" value={taxBracket} onChange={(e) => setTaxBracket(+e.target.value)} />
                  </Field>
                </CardContent>
              </Card>
              ...
              <Card className="border-t-[3px] border-t-emerald-500">
                ...
                <CardContent className="flex flex-col gap-3">
                  <Field label="Monthly Extra (₹)">
                    <Input type="number" value={monthlyExtra} onChange={(e) => setMonthlyExtra(+e.target.value)} />
                  </Field>
                  ...
                  <div className="flex flex-col gap-1.5">
                    ...
                    <ToggleGroup type="single" value={strategy} onValueChange={(v) => v && setStrategy(v)}>
                  ...
                  {loanType === "fixed" && (
                    <Field label="Preclosure Penalty" suffix="%">
                      <Input type="number" value={fixedPenalty} onChange={(e) => setFixedPenalty(+e.target.value)} />
                    </Field>
                  )}
                </CardContent>
              </Card>
